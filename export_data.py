...
data_dict["defaults_df"] = defaults_df
...
data_dict["defaults_12"] = out_df.groupby(['id_number']).agg({"age": "max", "Default?": "max"}).rename({"age": "Age (months)"}, axis=1)
...
data_dict['clients_scores'] = clients_df.copy()
...
def save_defaults_df(path):
    data_dict['defaults_df'].to_csv(path, index=False)


def save_defaults_12(path):
    data_dict['defaults_12'].to_csv(path)


def save_clients_scores(path):
    data_dict['clients_scores'][['id_number', 'default', 'score']].to_csv(path, index=False)


def save_age_woe_plot(path):
...
def save_age_of_car_woe_plot(path):
    column_name = 'age_of_car_category'
    df_gb = clients_df.groupby([column_name]).agg({"default": "sum",
                                                   "event": lambda x: x.sum() / clients_df.event.sum(),
                                                   "non_event": lambda x: x.sum() / clients_df.non_event.sum()})
    df_gb['woe'] = np.log(np.clip(df_gb.event / df_gb.non_event, 0.001, np.inf))
...
def get_woe_iv_risk_horizon(risk_horizon, column_name):
    out_df = payments_df[payments_df.age == risk_horizon][['contract_number',
                                                           'date',
                                                           'contract_date',
                                                           'id_number',
                                                           'age']].reset_index(drop=True)  # Берем только нужные поля и "сбрасываем" индекс (начинаем с 0)
    ...
    temp_df = out_df.groupby(['id_number'], as_index=False).agg({"age": "max", "Default?": "max"})
    temp_clients_df = clients_df[clients_df.id_number.isin(temp_df.id_number.unique())]
    df_gb = temp_clients_df.groupby([column_name]).agg({"default": "sum",
                                                        "event": lambda x: x.sum() / temp_clients_df.event.sum(),
                                                        "non_event": lambda x: x.sum() / temp_clients_df.non_event.sum()})
    ...
    plt.xlabel(column_name, fontsize=14)
    plt.title("%s IV plot" % column_name, fontsize=16)

    return df_gb, woe_fig, iv_fig
